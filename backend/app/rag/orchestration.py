...
from backend.app.ai.providers import BaseLLMProvider, DeterministicLLMProvider
...
def aggregate_candidates(query: str, requirements: Requirements, *, tenant_id: str, store: VectorStore, provider: EmbeddingProvider, top_k: int = 5, llm: BaseLLMProvider | None = None) -> list[RagResult]:
    ...
    for record, score in retrieve_candidates(query, tenant_id=tenant_id, store=store, provider=provider, top_k=top_k):
        ...
        results.append(RagResult(resume_id=record.document_id, normalized_score=max(0, min(1, score)), matching_skills=matching, missing_skills=missing, experience_comparison="Experience requires human review", explanation=explanation, confidence=max(0, min(1, score)), evidence=evidence))
    return results
